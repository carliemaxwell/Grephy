...
public class DFA {
    ...
    public static List<DFATransition> dfaTransitions;
...
//        System.out.println("Hashmap of states w/ eclosures " + statesAndEclosures);
    ...
    public static void findSubset(NFA nfa, List<Integer> eclosure) {
        ...
        for (int y = 0; y < Ingestion.alphabet.size(); y++) {
            ...
            dfaTransitions.add(new DFATransition(eclosure, newSubset, currentChar));
    ...
    public static DFA createDFA(NFA nfa) throws IOException {
        ...
        for (int y = 0; y < Ingestion.alphabet.size(); y++) {
            ...
            dfaTransitions.add(new DFATransition(eclosure, returnedStates, currentChar));
...
//            System.out.println("new state size " + newStates.size());
    ...
    public static void testDFA(String line) {
        ...
        for (int y = 0; y < line.length(); y++) {
            for (int x = 0; x < dfaTransitions.size(); x++) {
                if (dfaTransitions.get(x).prior.equals(startState)
                        && dfaTransitions.get(x).label == line.charAt(y)) {
        ...
        if (acceptingState.contains(startState)) {
